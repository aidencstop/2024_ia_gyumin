...
def counselor_editstudents(request, id):
    ...
    if request.method == 'POST':
        if 'save' in request.POST:
            ...
            ib_subject = ib_subject_dict[student_instance.major.name]
            ...
            # get IB IA title
            title_query = "Recommend me a title for IB IA for " + ib_subject + ". The subject should be appropriate for high-school student to research and should be related with " + student_instance.major.name.lower() + ". Just answer one clear title only(it must not have anything like '[specific topic]'), and don't say anything else."
            ...
            existing_subjects = ActivityExperience.objects.filter(student=student_instance,
                                                                  category=category_subjects).order_by(
                'pk')
            existing_awards = ActivityExperience.objects.filter(student=student_instance, category=category_awards).order_by(
                'pk')
            existing_isa = ActivityExperience.objects.filter(student=student_instance, category=category_isa).order_by('pk')
            existing_ea = ActivityExperience.objects.filter(student=student_instance, category=category_ea).order_by('pk')
            existing_va = ActivityExperience.objects.filter(student=student_instance, category=category_va).order_by('pk')

            existing_subjects_names = [es.activity.name for es in existing_subjects]
            subjects_strength, fail_subject_list = calculate_strength_with_subjects(student_instance.major.name,
            ...
            if subjects_strength == 1:
                subjects_query = "1. Please write a sentence stating that the courses the student has currently completed are sufficient to apply to the desired major, \'" + student_instance.major.name + "\'. You must also mention that the student's " + \
                                 activity_trait_dict[
                                     'subjects'] + " is clearly revealed through the subjects completed."
            else:
                subjects_query = "1. Please write a sentence stating that the courses the student has currently completed are not sufficient to apply to the desired major, \'" + student_instance.major.name + "\'. You must also mention that the student's " + \
                                 activity_trait_dict[
            ...
            if awards_strength == 1:
                awards_query = "2. Please write a sentence stating that the current student's awards are sufficient to apply to the desired school, " + student_instance.university.name + ". Please also mention that the student's award history reflects the student's " + \
                               activity_trait_dict['awards'] + "."
            else:
                awards_query = "2. Please write a sentence stating that the current student's award history is not sufficient to apply to the desired school, " + student_instance.university.name + ". Please also mention that adding a few more awards will highlight the student's " + \
                               activity_trait_dict['awards'] + "."
            ...
            if isa_strength == 1:
                isa_query = "3. Please write a sentence explaining that the student's in-school activities meet the standards of the desired school, " + student_instance.university.name + ". Please also mention that the student's in-school activities can show the student's " + \
                            activity_trait_dict['isa'] + "."
            else:
                isa_query = "3. Please write a sentence explaining that the student's in-school activities are somewhat lacking compared to the standards of the desired school, " + student_instance.university.name + ". Please also mention that if the student adds in-school activities, the student can show the student's " + \
                            activity_trait_dict['isa'] + "."
            ...
            if ea_strength == 1:
                ea_query = "4. Write a sentence explaining that the student's extracurricular activities meet the standards of the desired school, " + student_instance.university.name + ". Also mention that the student's extracurricular activities can show the student's " + \
                           activity_trait_dict['ea'] + "."
            else:
                ea_query = "4. Write a sentence explaining why the student's extracurricular activities fall short of the standards of the desired school, " + student_instance.university.name + ". Please also mention that if the student adds extracurricular activities, it can show the student's " + \
                           activity_trait_dict['ea'] + "."
            ...
            if va_strength == 1:
                va_query = "5. Please write a sentence explaining that your student's volunteer activities meet the standards of the desired school, " + student_instance.university.name + ". Also mention that the student's volunteer activities show the student's " + \
                           activity_trait_dict['va'] + "."
            else:
                va_query = "5. Please write a sentence explaining why the student's volunteer activities do not meet the standards of the desired school, " + student_instance.university.name + ". Please also mention that if the student adds volunteer activities, it can show the student's " + \
                           activity_trait_dict['va'] + "."

            total_query += va_query

            # get recommendation
            recommendation_query = "I decided to write IB IA for the subject \'" + ib_subject + "\' with the following title.\n\'" + ib_ia_title + "\'\n And the university major I want to pursue is " + student_instance.major.name + "." + " At this time, write a sentence explaining why the IA topic you have chosen is suitable for the subject and the major you are applying for, and explaining what kind of content you should write about. The format of the sentence should be as follows(must follow the line-breakings):\n The topic chosen is ~~in one respect~~ suitable as an IA topic for the subject.\n Also, in one respect, it is largely related to the major that the student is pursuing.\n If student conducts research and writes in a way that ~~, the student will be able to produce good results."
            ...
            student_instance.ib_subject = ib_subject
            student_instance.ib_ia_title = ib_ia_title
            student_instance.strength = "^&*".join(strength)
            student_instance.weakness = "^&*".join(weakness)
            student_instance.recommendation = recommendation
...
def counselor_studdashboarddetail(request, id):
    student = Student.objects.get(id = id)
    ib_subject = student.ib_subject
    ib_ia_title = student.ib_ia_title
    strength = student.strength.split("^&*")
    weakness = student.weakness.split("^&*")
